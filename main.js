...
function arrayCombos(arrayIn){
    // YOUR CODE HERE
    let returnArray = [];
    for(i = 0; i < arrayIn.length; i++){
      let HoldMyBeerArray = []
      for(j = 0; j < arrayIn.length; j++){
        HoldMyBeerArray.push(arrayIn[i] + arrayIn[j])
      }
      returnArray.push(HoldMyBeerArray)
    }
    return returnArray;
}
...
function navigateRushHour( timeOfDay ){
    ...
    //Return if they should go around or through
}
...
let factorial = function factorially( num ){
    // YOUR CODE HERE
    //Take in a NUMBER
    //Start at 1, then times it by the next number, store result
    //Times by store number
    //Repeat till NUMBER
    let a = 1
    for(i=1; i < num;i++){
      a = a*i
...
let interpolate = function interpolateFUNKY( start, end, count ){
    // YOUR CODE HERE
    //Store results in an array
    let resultArray = []
    //Calculate the distance betwen the two numbers
    let distance = end - start
    //Divide the distance by the count of interpolation
    let interval = distance / count
    //Result should be added to start and times count as many times as count exists
    for (i=0;i<=count;i++){
      resultArray.push(interval*i + start)
    }
    return resultArray
}
